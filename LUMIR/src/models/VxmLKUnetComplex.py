...
class VxmLKUnetComplex(nn.Module):
    ...
    def __init__(self,
        ...
        super(VxmLKUnetComplex, self).__init__()
        ...
        print("start_channel: %d" % (self.start_channel))
    ...
    def forward(self, x, y, x_seg=None, y_seg=None, registration=False, out_pre_resize=False):
        ...
        # print(f_xy.shape)
        #f_yx = self.dc10(d3)
        f_xy_0 = torch.nn.functional.interpolate(f_xy[:,0:1,...], size=self.size, mode='trilinear')
        f_xy_1 = torch.nn.functional.interpolate(f_xy[:,1:2,...], size=self.size, mode='trilinear')
        f_xy_2 = torch.nn.functional.interpolate(f_xy[:,2:3,...], size=self.size, mode='trilinear')
