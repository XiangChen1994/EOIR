...
class dispWarp(nn.Module):
    ...
    def forward(self, e, d, st_e, st_d, up_field=None):
        ...
        up_field = self.up_tri(field) 
        up_field = up_field *torch.Tensor([[2,2,1]]).cuda().unsqueeze(2).unsqueeze(3).unsqueeze(4).repeat(up_field.shape[0],1,up_field.shape[2],up_field.shape[3],up_field.shape[4])
        
        #print(e_x.shape,up_field.shape,field.shape,torch.Tensor([[2,2,1]]).unsqueeze(2).unsqueeze(3).unsqueeze(4).repeat(up_field.shape[0],1,up_field.shape[2],up_field.shape[3],up_field.shape[4]).shape)
        #input()
...
class hmorph_heart(nn.Module):
    ...
    def __init__(self,
        ...
        super(hmorph_heart, self).__init__()
        ...
        self.disp_warp_4 = dispWarp(N_s * 8, 3, ks, nn.Identity, self.is_int, [s // 16 for s in ss[:2]]+[ss[2]])
        self.disp_warp_3 = dispWarp(N_s * 4, 3, ks, nn.Identity, self.is_int, [s // 8 for s in ss[:2]]+[ss[2]])
        self.disp_warp_2 = dispWarp(N_s * 2, 3, ks, nn.Identity, self.is_int, [s // 4 for s in ss[:2]]+[ss[2]])
        self.disp_warp_1 = dispWarp(N_s * 2, 3, ks, nn.Identity, self.is_int, [s // 2 for s in ss[:2]]+[ss[2]])
